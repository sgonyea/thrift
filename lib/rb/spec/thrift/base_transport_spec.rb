...
module Thrift
  describe "Thrift::Transport" do
    ...
    describe MemoryBufferTransport do
      let!(:buffer) { MemoryBufferTransport.new }
    ...
    describe IOStreamTransport do
      let!(:input)  { mock("Input", :closed? => false) }
      let!(:output) { mock("Output", :closed? => false) }
      let!(:trans)  { IOStreamTransport.new(input, output) }
