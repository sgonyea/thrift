...
module Thrift
  describe "ThriftServerSpec" do
    include Thrift

    let(:processor) { mock("Processor") }
    let(:serverTrans) { mock("ServerTransport") }
    let(:trans) { mock("BaseTransport") }
    let(:prot) { mock("BaseProtocol") }
    let(:client) { mock("Client") }
    let(:server) { server_type.new(processor, serverTrans, trans, prot) }

    describe BaseServer do
      let!(:server) { BaseServer.new(mock("Processor"), mock("BaseServerTransport")) }
    ...
    describe ThreadPoolServer do
      let(:threadQ) { mock("SizedQueue") }
      let(:excQ) { mock("Queue") }
      let(:server_type) { ThreadPoolServer }
